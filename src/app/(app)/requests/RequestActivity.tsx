...
import { History, Pencil, PlusCircle, Repeat } from "lucide-react";
import type { RequestActivity as RequestActivityEntry } from "@/lib/types";
...
const EVENT_META = {
  ...
  status_changed: { label: "Status changed", icon: Repeat },
  edited: { label: "Edited", icon: Pencil },
} as const;

function ActivityRow({ entry }: { entry: RequestActivityEntry }) {
...
// Read-only feed of what happened to a request/idea -- creation, status
// moves, and edits to the fields that matter. System-generated (unlike
// RequestComments, which is freeform and editable), so there's no
// composer here, just the list, newest last so it reads top-to-bottom
// like a history.
export default function RequestActivity({ activity }: { activity: RequestActivityEntry[] }) {
