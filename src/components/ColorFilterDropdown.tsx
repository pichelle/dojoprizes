      ...
      <button
        ...
        className={`flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm transition-colors ${
          selected.length > 0
            ...
            : "border-border-warm-strong bg-card text-ink hover:bg-nav-hover"
        }`}
      >
        {label}
        {selected.length > 0 && (
      ...
      {open && (
        <div className="scroll-warm absolute z-20 mt-1 w-52 max-h-64 overflow-y-auto bg-card border border-border-warm rounded-md shadow-md p-2">
          ...
          {options.map((o) => (
            <label
              key={o.value}
              className="flex items-center gap-2 text-sm text-ink cursor-pointer rounded-md px-2 py-1.5 transition-colors hover:bg-nav-hover"
            >
              <input
                type="checkbox"
