    ...
    <>
      ...
      >
        ...
        <button
          ...
          aria-label="More"
          className="flex-1 flex flex-col items-center gap-1 pt-2.5 pb-2 text-[11px] text-ink-soft font-medium"
        >
      ...
      {moreOpen && (
        <div className="sm:hidden fixed inset-0 z-40">
          <div
            className="absolute inset-0 bg-ink/20 overlay-fade-in"
            onClick={() => setMoreOpen(false)}
            aria-hidden="true"
          />
          <div className="slide-up absolute left-0 right-0 bottom-0 bg-card border-t border-border-warm-strong rounded-t-[24px] shadow-[0_-8px_28px_rgba(58,58,56,0.16)] p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] space-y-1">
            <div className="flex items-center justify-between mb-1">
              <span className="font-serif text-lg text-ink px-1">More</span>
              <button
                type="button"
                onClick={() => setMoreOpen(false)}
                aria-label="Close"
